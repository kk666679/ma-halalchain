...
export async function classifyIngredient(name: string): Promise<Classification> {
  ...
  try {
    ...
    logger.warn({ error, name }, 'ML classification failed, using fallback');
    return {
      status: 'Unknown',
      confidence: 0.3,
...
function runHeuristicInference(embedding: number[]): Classification {
...
export async function trainClassifier(_trainingData: any[]) {
